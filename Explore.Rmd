...
<<<<< Add more saying that we didn't get anything from the numerical output, but the investigation helped us to find the root cause of the problematic clog-log plot! >>>>>

## Final Results/Summary

For this project we decided to explore if and how a husband's education affects divorce rates over time while taking into account the race of the couple. Our goal was to find the most significant model using various statistical methods that we learned in class as well as from our textbook. We determined that education definitely has an impact on divorce rate and then tested how other variables contributed to our model. For our final model we chose to use the education and stratified couple variables as it gives us our most significant p-value and lowest BIC. Regarding the education coefficients we see that with a bachelor's education the marriage roughly fails 1.4 times faster than with a highschool education and having a Master's degree or above results in the same failure rate as with a highschool education. Although we cannot know with certainty why we ended up with the above results, we think that there is less complications between spouse's when the husband has a lower education because it is likely that his partner has a similar education whereas the variance between the spouse's education is higher when the husband has bachelor's degree. When the husband has a master's degree or above the same reason as for the highschool education applies.To have more evidence to determine whether our claim is accurate we would need further exploration with the wife's education and income for both husband and wife as additional variables. In conclusion, according to our results we would recommend pursuing a Master's degree to ensure a more successful marriage.
