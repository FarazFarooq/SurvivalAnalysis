...
Our team primarily interested in how the husband's education level affects time until divorce, using the various racial covariates as controlling variables. Our secondary goal was to investigate the influence of racial makeups as a stratified variable.

## Methodology

After an exploratory analysis, our team began with a base Cox-PH model including only the husband's education level. We then constructed various control models using the racial covariates. After establishing that the base model is still significant under the controls, we turned our interest to the possible influence of these racial predictors as stratified variables.
...
# One library is not on CRAN, using devtools we can install it from GitHub
# devtools::install_github("sachsmc/ggkm")
# https://github.com/sachsmc/ggkm

# Load data, working directory needs to be set
...
**Bottom Right** The *couple* predictor gives us a greater insight than *mixed*. Interestingly, we can see that couples with neither partner being black have a higher survival function. 
...
Here we add the racial covariates to previous model with education. Our goal is to ensure education is a significant predictor in all models.
...
The R output above confirms that *education* is a significant predictor in all control situations. Now that we have established that education alone is a stable covariate, our team decided to investigate the above models further. Our motivation is that one (or more) of the models may be a better choice than *education* alone.
...
# Model using new couple covariate
...
Although the BIC on the model using *education* and *mixed* was the lowest, we decided to continue our analysis using a new variable, *couple* which encodes all the racial makeups into one predictor. Our justification is that *couple* holds far more information than *mixed* and may provide a more insightful model. 

Below, we use a likelihood ratio test to ensure there is no significant difference between the model using the newly encoded predictor, and the model containing the individual predictors being encoded.
...
This allows us to use a more condensed predictor for later analysis. Lastly, it is interesting to note that the model using Education + Mixed + Hblack + Wblack is in fact the same model as Education + Couple.
...
**Right** Again, the intersecting lines are concerning. In this case, however, we will investigate the necessity of stratifying the racial makeup predictor.
...
The numerical tests show that using *couple* as a stratified variable improves on the previous model using education and couple (as a non-stratified variable).

```{r, echo = F}
contors1 <- data.frame(couple = as.factor(c("BB", "BB", "BB", "BO", "BO", "BO", "OB", "OB", "OB", "OO", "OO", "OO")), 
                       edu = as.factor(c(0, 1, 2, 0, 1, 2, 0, 1, 2, 0, 1, 2)) )
edu_strata_km <- survfit(edu_strata_coxph, newdata = contors1)
plot(edu_strata_km, col = 1:4, lty = c(rep(1,3), rep(4, 3), rep(8,3), rep(9,3)))
legend(50, 1, c("0", "1", "2", "BB", "BO", "OB", "OO"), col=c(1:4, 2, 2), lty = c(rep(1,3), rep(4, 3), rep(8,3), rep(9,3)))
title("Cox-PH Model with Stratification")
```

**But**, being the naive statisticians that we are, we did not realize the cartesian product between the factor levels of *couple* and *education* would result in an uninterpretable plot of the KM estimates. In the spirit of discovery, we checked the PH assumptions for the model using *education* and *mixed*. The tests confirmed that we did not have any violations

```{r}
em_coxph <- coxph(Surv(years,div) ~ strata(mixed) + edu, data=divorce)
contors2 <- data.frame(edu = as.factor(c(0,1,2,0,1,2)), mixed = as.factor(c(0,0,0,1,1,1)))
em_km <- survfit(em_coxph, newdata = contors2)
```
```{r, echo = F}
plot(em_km, ymin = .20, col = 1:3, lwd = 2, lty = c(rep(1,3), rep(4, 3)))
legend(50, 1, c("0", "1", "2", "Mixed", "Non-Mixed"), col=c(1:3, 2, 2), lty = c(1,1,1,1,4))
title("Cox-PH Model with Stratification")
```

*Note that the minimum y-axis value is 0.20.*  <<<<<<<< Interpet plot here. >>>>>>>>

## Time Transform

Observations split on time == 3.85, we will use this time transformation (instead of the more involved approach) because it does not introduce a new $\beta$.

```{r}
# Split on t = 3.85
edu_split_df <- survSplit(Surv(years, div) ~ edu + couple, data = divorce, cut = 3.85)
edu_surv_split <- coxph(Surv(years, div) ~ edu, data = edu_split_df)
