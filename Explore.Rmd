...
We also encoded two new variables using the above data,
...
**Bottom Right** <<<<<<<< FILL IN EXPLANARTION HERE >>>>>>>>
...
The R output above confirms that *education* is a significant predictor in all control situations. Now that we have established that education *alone* is a stable covariate, our team wanted to investigate the above models further. Our motivation is that one (or more) of the models may be a better choice than *education* alone.
...
Below, we use a log-rank test to ensure there is no significant difference between the model using the newly encoded predictor, and the model containing the individual predictors being encoded.
...
The numerical tests show that using *couple* as a stratified variable improves on the previous model using education and couple.
...
**But**, being the naive statisticians that we are, we did not realize the cartesian product between *couple* and *education* would result in an uninterpretable plot of the KM estimates. In the spirit of discovery, we checked the PH assumptions for the model using *education* and *mixed*. The tests confirmed that we did not have any violations
