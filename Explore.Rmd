...
Our team decided to build our model bottom-up, meaning we fit single.... ababasdhashdaabs
...
blog <- ggplot(divorce, mapping = aes(time = years, status = div, color = factor(edu)))  +
...
clog <- ggplot(divorce, mapping = aes(time = years, status = div, color = factor(mixed)))  +
  geom_km(trans = "cloglog") + 
  ylab("Complementary Log-Log") + 
  ggtitle("Log-Log curve for Mixed \n Non-Mixed Couples") +
  ylab("Y") +
  scale_x_log10() +
  theme(legend.title = element_blank())
 
dlog <- ggplot(divorce, mapping = aes(time = years, status = div, color = factor(hblack)))  +
  geom_km(trans = "cloglog") + 
  ylab("Complementary Log-Log") + 
  ggtitle("Log-Log curve for Black \n Non-Black Husbands") +
  ylab("Y") +
  scale_x_log10() +
  theme(legend.title = element_blank())

plot_grid(blog, clog, dlog, nrow = 2, ncol = 2, label_size = 10)
```

The clog-log plots above help us check our proportional hazards assumption graphically. Though it is difficult to infer any violations from the plots, the bottom left plot is especially concerning.
...
hblack_coxph <- coxph(Surv(years, div) ~ hblack, data=divorce)
...
# GLOBAL is tst for whole model's proportionality
```

```{r}
emb_coxph <- coxph(Surv(years, div) ~ edu + mixed + hblack, data=divorce)
test <- cox.zph(emb_coxph)
```

Looking at the data, we are concerned with the log-log graph for Black and Non-Black husbands. We can confirm that our column hblack1 violates the proportionality assumption. 

elaborate small p-values, baseline hazard rate is only dependent on time? 


## Modeling

... Merge with Modeling.Rmd here ...
